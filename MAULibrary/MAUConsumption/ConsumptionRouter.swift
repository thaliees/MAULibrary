...
enum ConsumptionRouter: URLRequestConvertible {
    ...
    var method: HTTPMethod {
        switch self {
        case .requestAccessToken, .createUserProfile, .savePrivacyPolicyResponse, .getARCO, .getREUS, .sendSMSToken, .resendSMSToken, .sendEmailToken, .resendEmailToken, .validateToken, .getSessionId:
            return .post
        default:
            return .get
        }
    ...
    var path: String {
        ...
        case .getSessionId:
            return MAUPaths.identityVerification
        }
    }
